...
async function run (pwa, _emitAssets) {
  ...
  } else if (options.purpose.constructor === Array) {
    const valid_purpose = ['badge', 'maskable', 'any']
    options.purpose = options.purpose.filter(item => valid_purpose.includes(item))
  } else {
