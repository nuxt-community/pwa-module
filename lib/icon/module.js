...
async function run (pwa, _emitAssets) {
  ...
  const defaults = {
    ...
    iosSizes: [
      ['ipad', 1536, 2048], // Ipad
      ['ipadpro1', 1536, 2048], // Ipad Pro 9.7"
      ['ipadpro2', 1668, 2224], // Ipad Pro 10.5"
      ['ipadpro3', 2048, 2732], // Ipad Pro 12.9"
      ['iphonese', 640, 1136], // Iphone SE
      ['iphone6', 750, 1334], // Iphone 6
      ['iphoneplus', 1080, 1920], // Iphone Plus
      ['iphonex', 1125, 2436], // Iphone X
      ['iphonexr', 828, 1792], // Iphone XR
      ['iphonexsmax', 1242, 2688] // Iphone XS Max
    ],
    targetDir: 'icons',
  ...
  // Generate IOS icons
  await generateIosSplash.call(this, options)
...
async function generateIosSplash (options) {
  // Get hash of source image
  if (!options.iconHash) {
    options.iconHash = await hasha.fromFile(options.iconSrc).then(h => h.substring(0, 6))
  }
  ...
  for (const size of options.iosSizes) {
    options._iosSplash[size[0]] = `${options.targetDir}/icon_${size[0]}.${options.iconHash}.png`
  }

  // Generate _assetIosSplash
  options._assetIosSplash = {}
  options.iosSizes.forEach((size) => {
    options._assetIosSplash[size[0]] = joinUrl(options.publicPath, options._iosSplash[size[0]])
  })
}
...
function addManifest (options, pwa) {
  ...
  if (!pwa.manifest.iosSplash) {
    pwa.manifest.iosSplash = {}
  }

  pwa.manifest.icons.push(...options._assetIcons)
  pwa.manifest.iosSplash = Object.assign(pwa.manifest.iosSplash, options._assetIosSplash)
}
      ...
      config.plugins.push({
        apply (compiler) {
          compiler.hooks.emit.tapPromise('nuxt-pwa-icon', async (compilation) => {
            ...
            // Ios splash
            await Promise.all(options.iosSizes.map(async (size) => {
              const targetFilename = options._iosSplash[size[0]]
              const srcFileName = path.join(options._cacheDir, `${size[0]}.png`)
              const src = await fs.readFile(srcFileName)
              compilation.assets[targetFilename] = { source: () => src, size: () => src.length }
            }))
          })
