...
const { find, getRouteParams, joinUrl } = require('../utils')
...
function addOneSignal (oneSignalOptions) {
  ...
  if (!find(this.options.head.script, 'hid', 'onesignal')) {
  ...
  const makeSW = (name, scripts) => {
    ...
    writeFileSync(path.resolve(this.options.srcDir, this.options.dir.static, name), workerScript, 'utf-8')
  }
