...
options.runtimeCaching.forEach(r => {
  ...
  if (r.strategyOptions) {
    ...
    if (r.strategyOptions.plugins) {
      strategyOptionsCode += ',plugins:['
      strategyOptionsCode += r.strategyOptions.plugins.map(p => {
        if ('use' in p) {
          // using built-in plugins
          return 'new workbox.' + pluginModules[p.use] + '(' + p.config.map(param => toCode(param)).join(',') + ')'
        } else {
          // custom (self-built) plugins
          return toCode(p)
        }
      }).join(',')
      strategyOptionsCode += ']'
    ...
    let options = Object.assign({}, r.strategyOptionsCode)
    delete options.plugins
    strategyOptionsCode += toCode(options).substring(1, -1)
